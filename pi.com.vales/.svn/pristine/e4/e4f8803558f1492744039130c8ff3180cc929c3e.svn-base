...
public class ConsultaUsuario extends JFrame{
	 ...
	 */
	public ConsultaUsuario(CadUsuario tela){
		super("Consulta Usuario");
		...
		buscar.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if((local != null) && (local.trim().length() != 0)){
					lista = DAO.geral(null, local, "nomeUsuario");
					if(lista == null){
						lista = DAO.geral(local, null, "codigoUsuario");
						if(lista == null){
							tResposta.setText("Nenhum Registro Encontrado");
						}
					}
				}else{
					lista = DAO.geral(null, null, "codigoUsuario");
				}
				
				if(lista != null){
					String[] linha = null;
					for (Iterator<UsuarioVO> iter = lista.iterator(); iter.hasNext();) {
						UsuarioVO vo = iter.next();
						
						linha = new String[]{String.valueOf(vo.getCodigoUsuario()), vo.getNomeUsuario(), vo.getNivelUsuario()};
