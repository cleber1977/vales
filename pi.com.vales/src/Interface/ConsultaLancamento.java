...
public class ConsultaLancamento extends JFrame {
	 ...
	 */
	public ConsultaLancamento(Lancamento tela){
		super("Consulta Lancamento");
		...
		buscar.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if((local != null) && (local.trim().length() != 0)){
					lista = DAO.geral(null, local, "descricaoLanc");
					if(lista == null){
						lista = DAO.geral(local, null, "codigoLanc");
						if(lista == null){
							tResposta.setText("Nenhum Registro Encontrado");
						}
					}
				}else{
					lista = DAO.geral(null, null, "codigoLanc");
				}
				
				if(lista != null){
					String[] linha = null;
					for (Iterator<LancamentoVO> iter = lista.iterator(); iter.hasNext();) {
						LancamentoVO vo = iter.next();
						
						linha = new String[]{String.valueOf(vo.getCodigoLanc()), vo.getDescricaoLanc(), vo.getNomeFun(), String.valueOf(vo.getValorLanc()), String.valueOf(vo.getDataVencBusca())};
