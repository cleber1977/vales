...
public class ConsultaFuncionario extends JFrame{
	...
	private CadFuncionario telaPai;
	 ...
	 */
	public ConsultaFuncionario(CadFuncionario tela){
		super("Consulta Funcionario");
		telaPai = tela;
		...
		buscar.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if((local != null) && (local.trim().length() != 0)){
					lista = DAO.geral(null, local, "nomeFun");
					if(lista == null){
						lista = DAO.geral(local, null, "codigoFun");
						if(lista == null){
							tResposta.setText("Nenhum Registro Encontrado");
						}
					}
				}else{
					lista = DAO.geral(null, null, "codigoFun");
				}
				
				if(lista != null){
					String[] linha = null;
					for (Iterator<FuncionarioVO> iter = lista.iterator(); iter.hasNext();) {
						FuncionarioVO vo = iter.next();
						
						linha = new String[]{String.valueOf(vo.getCodigoFun()), vo.getNomeFun(), vo.getCpfFun(), vo.getFoneFun()};
